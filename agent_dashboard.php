...
// Count unread lot status records
...
if ($historyCheck && $historyCheck->num_rows > 0) {
  ...
  // Count unread records for this agent
  $hasViewings = false;
  $hasLotsAgent = false;
  ...
  if ($hasViewings) {
    $sql = "SELECT COUNT(*) as c FROM lot_status_history h LEFT JOIN lots l ON l.id = h.lot_id WHERE h.event_type = 'surrender' AND h.is_read = 0 AND (EXISTS (SELECT 1 FROM viewings v WHERE v.lot_id = h.lot_id AND v.agent_id = ? AND LOWER(TRIM(v.client_email)) = LOWER(TRIM(h.previous_owner_email)))";
    if ($hasLotsAgent) {
      $sql .= " OR l.agent_id = ?";
    }
    $sql .= ")";
    ...
    if ($stmt = $conn->prepare($sql)) {
      ...
      $res = $stmt->get_result();
      if ($row = $res->fetch_assoc()) {
        $unreadLotStatusCount = (int)($row['c'] ?? 0);
      }
      $stmt->close();
    }
  } elseif ($hasLotsAgent) {
    $sql = "SELECT COUNT(*) as c FROM lot_status_history h LEFT JOIN lots l ON l.id = h.lot_id WHERE h.event_type = 'surrender' AND h.is_read = 0 AND l.agent_id = ?";
    if ($stmt = $conn->prepare($sql)) {
      $stmt->bind_param('i', $agentId);
      $stmt->execute();
      $res = $stmt->get_result();
...
function getUpcomingPaymentsForAgent(mysqli $conn, int $agentId): array {
  $upcomingPayments = [];
  
  // Check for columns that determine agent assignment
  ...
  // Build viewing agent joins similar to commission records query
  ...
  // Query for lots with upcoming payments
  $sql = "
    SELECT
      l.id,
      l.block_number,
      l.lot_number,
      COALESCE(ll.location_name, 'Unknown') AS location_name,
      l.owner_id,
      COALESCE(NULLIF(TRIM(u.first_name), ''), '') AS client_first_name,
      COALESCE(NULLIF(TRIM(u.last_name), ''), '') AS client_last_name,
    ...
    $viewingAgentJoinByLotId
    $viewingAgentJoinByLotNo
    WHERE ($assignmentWhere)
      AND IFNULL(l.payment_amount, 0) > 0
      AND l.payment_deadline IS NOT NULL AND TRIM(l.payment_deadline) != ''
      AND (
        l.status IN ('Installments', 'Reserved', 'Sold')
        OR COALESCE(NULLIF(l.payment_type, ''), '') IN ('Down Payment', 'Fully Paid', 'Installments')
      )
  ...
  if ($stmt = $conn->prepare($sql)) {
    ...
    while ($row = $res->fetch_assoc()) {
      ...
      $paymentDeadline = trim((string)($row['payment_deadline'] ?? ''));
      $dueDay = (int)($row['payment_due_day'] ?? 0);
      
      // Skip if no payment amount or deadline
      if ($monthlyAmount <= 0 || $paymentDeadline === '') {
        continue;
      }
      
      // Calculate due installments and next due date
      $dueInstallments = countDueInstallments($paymentDeadline, $dueDay, $today);
      $paidInstallments = (int)floor(($totalPaid + 0.0001) / $monthlyAmount);
      $outstanding = $dueInstallments - $paidInstallments;
      
      // Calculate next due date
      try {
        $startDate = new DateTime($paymentDeadline);
        $cursor = new DateTime($startDate->format('Y-m-01'));
        $daysInMonth = (int)$cursor->format('t');
        $effectiveDueDay = min($dueDay > 0 ? $dueDay : (int)$startDate->format('j'), $daysInMonth);
        $cursor->setDate((int)$cursor->format('Y'), (int)$cursor->format('m'), $effectiveDueDay);
        
        if ($cursor < $startDate) {
          $cursor = clone $startDate;
        }
        
        // Move to the next unpaid installment date
        $monthCounter = 0;
        while ($monthCounter < $paidInstallments && $monthCounter < 600) {
          $cursor->modify('first day of next month');
          $daysInMonth = (int)$cursor->format('t');
          $cursor->setDate((int)$cursor->format('Y'), (int)$cursor->format('m'), min($effectiveDueDay, $daysInMonth));
          $monthCounter++;
        }
        ...
        $nextDueDate = $cursor;
        $daysUntilDue = (int)$today->diff($nextDueDate)->format('%r%a');
      } catch (Throwable $e) {
        continue;
      }
      
      // Skip if already paid or no outstanding amount
      if ($outstanding <= 0) {
        continue;
      }
      ...
      if ($daysUntilDue < 0) {
        $status = 'overdue';
        $statusLabel = 'Overdue by ' . abs($daysUntilDue) . ' day' . (abs($daysUntilDue) !== 1 ? 's' : '');
        $sortKey = abs($daysUntilDue) * -1;
      } elseif ($daysUntilDue <= 7) {
        $status = 'due_soon';
        $statusLabel = 'Due in ' . $daysUntilDue . ' day' . ($daysUntilDue !== 1 ? 's' : '');
        $sortKey = $daysUntilDue + 1000;
      } else {
        $status = 'upcoming';
        $statusLabel = 'Due in ' . $daysUntilDue . ' days';
        $sortKey = $daysUntilDue + 2000;
      }
      
      $upcomingPayments[] = [
        'lot_id' => (int)($row['id'] ?? 0),
        'block_number' => (string)($row['block_number'] ?? 'N/A'),
        'lot_number' => (string)($row['lot_number'] ?? 'N/A'),
        'location_name' => (string)($row['location_name'] ?? 'Unknown'),
        'property' => (string)($row['block_number'] ?? 'N/A') . ' Block ' . (string)($row['lot_number'] ?? 'N/A') . ' Lot',
        'client_name' => trim((string)($row['client_first_name'] ?? '') . ' ' . (string)($row['client_last_name'] ?? '')),
        'client_email' => (string)($row['client_email'] ?? ''),
        'monthly_amount' => $monthlyAmount,
        'outstanding_balance' => max(0, $lotPrice - $totalPaid),
        'next_due_date' => $nextDueDate->format('Y-m-d'),
        'next_due_date_formatted' => $nextDueDate->format('F j, Y'),
        'days_until_due' => $daysUntilDue,
        'status' => $status,
        'status_label' => $statusLabel,
        'outstanding_months' => max(1, $outstanding),
        'sort_key' => $sortKey,
      ];
    }
    $stmt->close();
  ...
  // Sort by status priority and days
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  // Mark all lot status records as read
  if (isset($_POST['action']) && $_POST['action'] === 'mark_lot_status_read') {
    $hasViewings = false;
    $hasLotsAgent = false;
    $viewingsCheck = $conn->query("SHOW TABLES LIKE 'viewings'");
    if ($viewingsCheck && $viewingsCheck->num_rows > 0) {
    ...
    if ($hasViewings) {
      $sql = "UPDATE lot_status_history h LEFT JOIN lots l ON l.id = h.lot_id SET h.is_read = 1 WHERE h.event_type = 'surrender' AND h.is_read = 0 AND (EXISTS (SELECT 1 FROM viewings v WHERE v.lot_id = h.lot_id AND v.agent_id = ? AND LOWER(TRIM(v.client_email)) = LOWER(TRIM(h.previous_owner_email)))";
      if ($hasLotsAgent) {
        $sql .= " OR l.agent_id = ?";
      }
      $sql .= ")";
      ...
      $sql = "UPDATE lot_status_history h LEFT JOIN lots l ON l.id = h.lot_id SET h.is_read = 1 WHERE h.event_type = 'surrender' AND h.is_read = 0 AND l.agent_id = ?";
      if ($stmt = $conn->prepare($sql)) {
        $stmt->bind_param('i', $agentId);
        $stmt->execute();
        $stmt->close();
...
if ($_SERVER['REQUEST_METHOD'] === 'GET' && isset($_GET['fetch'])) {
  if ($_GET['fetch'] === 'notifications') {
    ...
    if ($check && $check->num_rows > 0) {
        // Join with viewings table to get viewing status
        $stmt = $conn->prepare("
          SELECT 
            an.*,
            COALESCE(v.status, '') AS viewing_status
          FROM agent_notifications an
          LEFT JOIN viewings v ON v.id = an.viewing_id
          WHERE an.agent_id=?
...
/* ---- Upcoming client payments (DASHBOARD) ---- */
$upcomingPayments = getUpcomingPaymentsForAgent($conn, $agentId);
      ...
      <ul class="space-y-0.5 w-full flex-1" id="spa-nav" style="min-height:0; overflow:visible;">
        ...
        <li>
          <a href="#lot-status" data-target="section-lot-status"
             class="flex items-center px-8 py-3 rounded transition hover:bg-green-800 relative">
            <svg class="w-5 h-5 mr-3 text-white" viewBox="0 0 24 24" fill="currentColor">
              <path d="M4 6h16v2H4V6zm0 4h16v2H4v-2zm0 4h16v6H4v-6zm6 4h4v2h-4v-2z"/>
            </svg>
            <span>Lot Status</span>
            <?php if ($unreadLotStatusCount > 0): ?>
              <span style="margin-left:10px; background-color: #ef4444; color: white; font-size: 11px; font-weight: bold; padding: 2px 6px; border-radius: 50%; min-width: 20px; text-align: center; display: inline-block;">
  ...
  <main class="flex-1 p-8 mt-8" style="margin-left:280px;">
      ...
      <section class="mt-6">
        <div class="bg-white rounded-2xl border shadow p-6">
          <div class="flex flex-wrap items-center justify-between gap-3 mb-4">
            <div>
              <h3 class="text-xl font-bold text-gray-900">Upcoming Client Payments</h3>
              <p class="text-sm text-gray-500">Monitor when your clients' payments are due to ensure timely collection.</p>
            </div>
            <div class="text-sm text-gray-600">
              <?php 
                $overdueCnt = count(array_filter($upcomingPayments, fn($p) => $p['status'] === 'overdue'));
                $dueSoonCnt = count(array_filter($upcomingPayments, fn($p) => $p['status'] === 'due_soon'));
              ?>
              <span class="inline-block bg-red-100 text-red-800 px-3 py-1 rounded-full text-xs font-semibold mr-2">
                <?php echo $overdueCnt; ?> overdue
              </span>
              <span class="inline-block bg-yellow-100 text-yellow-800 px-3 py-1 rounded-full text-xs font-semibold">
                <?php echo $dueSoonCnt; ?> due soon
              </span>
            </div>
          </div>

          <?php if (empty($upcomingPayments)): ?>
            <div class="text-sm text-gray-500">All client payments are current. Great job! 🎉</div>
          <?php else: ?>
            <div style="max-height: 600px; overflow-y: auto;" class="space-y-3">
                ...
                <div class="border rounded-lg p-4 bg-gray-50 hover:bg-gray-100 transition">
                  <div class="flex items-start justify-between gap-3 mb-2">
                    <div>
                      <div class="font-semibold text-gray-900">
                        <?php echo h($payment['property']); ?>
                      </div>
                      <div class="text-sm text-gray-600">
                        <?php echo h($payment['location_name']); ?> | <?php echo h($payment['client_name']); ?>
                      </div>
                    </div>
                    <div>
                      <?php if ($payment['status'] === 'overdue'): ?>
                        <span class="inline-block bg-red-100 text-red-800 px-3 py-1 rounded-full text-xs font-semibold whitespace-nowrap">
                          <?php echo h($payment['status_label']); ?>
                        </span>
                      <?php elseif ($payment['status'] === 'due_soon'): ?>
                        <span class="inline-block bg-yellow-100 text-yellow-800 px-3 py-1 rounded-full text-xs font-semibold whitespace-nowrap">
                          <?php echo h($payment['status_label']); ?>
                        </span>
                      <?php else: ?>
                        <span class="inline-block bg-blue-100 text-blue-800 px-3 py-1 rounded-full text-xs font-semibold whitespace-nowrap">
                          <?php echo h($payment['status_label']); ?>
                        </span>
                      <?php endif; ?>
                    </div>
                  ...
                  <div class="grid grid-cols-2 gap-3 text-sm mb-3">
                    <div>
                      ...
                      <div class="font-semibold text-gray-900">PHP <?php echo number_format($payment['monthly_amount'], 2); ?></div>
                    </div>
                    <div>
                      <div class="text-gray-500">Outstanding Balance</div>
                      <div class="font-semibold text-gray-900">PHP <?php echo number_format($payment['outstanding_balance'], 2); ?></div>
                    </div>
                    <div>
                      <div class="text-gray-500">Next Due Date</div>
                      <div class="font-semibold text-gray-900"><?php echo h($payment['next_due_date_formatted']); ?></div>
                    </div>
                    <div>
                      <div class="text-gray-500">Outstanding Months</div>
                      <div class="font-semibold text-gray-900"><?php echo $payment['outstanding_months']; ?> month<?php echo $payment['outstanding_months'] !== 1 ? 's' : ''; ?></div>
                    </div>
                  </div>

                  <?php if (!empty($payment['client_email'])): ?>
                    <div class="flex gap-2">
                      <a href="mailto:<?php echo h($payment['client_email']); ?>?subject=Payment Reminder - <?php echo urlencode($payment['property']); ?>" 
                         class="flex-1 bg-green-600 hover:bg-green-700 text-white py-2 px-3 rounded text-sm font-medium transition text-center">
    ...
    <section id="section-lot-status" class="hidden">
      <div class="flex items-center justify-between mb-4">
        ...
        <?php if ($unreadLotStatusCount > 0): ?>
          <button onclick="markAllLotStatusAsRead()" class="bg-green-600 hover:bg-green-700 text-white py-2 px-4 rounded font-medium transition">
            Mark All as Read
          </button>
        <?php endif; ?>
      </div>
  ...
  fetch(window.location.pathname + '?fetch=notifications')
    ...
    .then(notifications => {
      ...
      container.innerHTML = notifications.map(n => {
        // Check if viewing is already approved or scheduled
        const isViewingApproved = n.viewing_status && (n.viewing_status.toLowerCase() === 'approved' || n.viewing_status.toLowerCase() === 'scheduled');
        ...
        <div class="mb-4 p-4 rounded border ${n.is_read ? 'bg-gray-50' : 'bg-green-50'}">
          ...
          <div class="mt-2 flex gap-2">
            ${(!n.is_read && Number(n.viewing_id || 0) > 0 && !isViewingApproved) ? `<button class=\"px-3 py-1 text-xs\" style=\"background:#22c55e !important; color:#fff !important; border:1px solid #16a34a !important;\" onclick=\"approveNotification(${n.id})\">Accept</button>` : ''}
            ${(!n.is_read && !isViewingApproved) ? `<button class=\"px-3 py-1 text-xs\" style=\"background:#dc2626 !important; color:#fff !important; border:1px solid #dc2626 !important;\" onclick=\"deleteNotification(${n.id})\">Decline</button>` : `<button class=\"px-3 py-1 text-xs\" style=\"background:#64748b !important; color:#fff !important; border:1px solid #64748b !important;\" onclick=\"markNotificationRead(${n.id})\">Mark as Read</button>`}
          </div>
        </div>
      `}).join('');
      refreshAgentNotificationBadge();
...
// Mark all lot status records as read
function markAllLotStatusAsRead() {
  ...
  fetch(window.location.href, {
    ...
    body: 'action=mark_lot_status_read&csrf_token=' + encodeURIComponent(csrfToken.value)
  })
  .then(response => response.json())
  .then(data => {
    if (data.success) {
