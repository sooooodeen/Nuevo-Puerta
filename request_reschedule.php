...
if ($agentEmail) {
    ...
    $bodyHtml = '<p>Hello ' . htmlspecialchars($agentName !== '' ? $agentName : 'Agent', ENT_QUOTES, 'UTF-8') . ',</p>'
        . '<p>A client requested a new viewing time.</p>'
        . '<p><strong>Requested time:</strong> ' . htmlspecialchars($preferred_at, ENT_QUOTES, 'UTF-8') . '</p>'
        . '<p><strong>Message:</strong><br>' . nl2br(htmlspecialchars($message !== '' ? $message : 'No message provided.', ENT_QUOTES, 'UTF-8')) . '</p>'
        . '<p>Please log in to the dashboard to confirm.</p>';
    $body = buildNovoPuertaEmailHtml(
        $subject,
        $bodyHtml,
        ['intro' => 'A viewing reschedule request has been submitted.', 'footer_note' => 'Review it in the agent dashboard as soon as possible.']
    );
    $headers = [
        'MIME-Version: 1.0',
    ...
    @mail($agentEmail, $subject, $body, implode("\r\n", $headers));
}
